...
export class OrganizationService {
  ...
  public async get(organization_id: string): Promise<IOrganization> {
    ...
    const organization_record = await this.model.findById(organization_id).lean().exec();
  ...
  public async getPopulated(organization_id: string): Promise<IOrganization> {
    ...
    const organization_record = await this.model
      ...
      .populate({ path: 'members.user', model: UserModel, select: '_id name lastName email' })
      .lean()
      .exec();
  ...
  public async update(organization: IOrganization): Promise<IOrganization> {
    ...
    const organization_record = await this.model.findByIdAndUpdate(organization._id, organization, {
      new: true,
    });
  ...
  public async exists(organization_id: string): Promise<boolean> {
    ...
    const exists = (await this.model.exists({ _id: organization_id }).lean().exec()) != null;
    return exists;
  }

  public async listAllFromUser(user_id: string): Promise<Array<IOrganization>> {
    //Check if mongoDB is connected
    checkConnectionStatus();
